...
public final class ThroughputLatencyServer extends DefaultRecoverable {
    String metricsFile = System.getProperty("metrics.file", "metrics.jsonl");
    ...
    public ThroughputLatencyServer(int id, int interval, int replySize, int stateSize,
        ...
        if (write > 0) {
            ...
            try {
                final File f =
                        File.createTempFile("bft-" + id + "-", Long.toString(System.nanoTime()));
        ...
        if (metricsFile != null) {
            ...
            measurements = root.putArray("measurements");
    ...
    public byte[][] appExecuteBatch(byte[][] commands, MessageContext[] msgCtxs,
        ...
        if (randomAccessFile != null) {
            ...
            try {
                ...
                Logger.getLogger(ThroughputLatencyServer.class.getName()).log(Level.SEVERE, null,
                        ex);
                ...
                try {
                    ...
                    Logger.getLogger(ThroughputLatencyServer.class.getName()).log(Level.SEVERE,
                            null, ex);
                }
            }
        }
    ...
    public byte[] execute(byte[] command, MessageContext msgCtx) {
        ...
        try {
            ...
            if (signed > 0) {
                ...
                if (signed == 1) {
                    ...
                    eng.initVerify(
                            replica.getReplicaContext().getStaticConfiguration().getPublicKey());
                } else {
        ...
        if (msgCtx != null && msgCtx.getFirstInBatch() != null) {
            ...
            totalLatency.store(
                    msgCtx.getFirstInBatch().executedTime - msgCtx.getFirstInBatch().receptionTime);
        ...
        if (iterations % interval == 0) {
            if (context)
                System.out.println("--- (Context)  iterations: " + iterations + " // regency: "
                        + msgCtx.getRegency() + " // consensus: " + msgCtx.getConsensusId()
                        + " ---");

            System.out.println(
                    "--- Measurements after " + iterations + " ops (" + interval + " samples) ---");

            tp = (float) (interval * 1000
                    / (float) (System.currentTimeMillis() - throughputMeasurementStartTime));
            ...
            if (jsonWriter != null) {
                ...
                throughput.put("max", maxTp);
                
                ObjectNode latency = m.putObject("latency");
                latency.put("unit", "us");

                writeLatencyJson(latency, totalLatency, "total");
                writeLatencyJson(latency, consensusLatency, "consensus");
                writeLatencyJson(latency, preConsLatency, "pre_consensus");
                writeLatencyJson(latency, posConsLatency, "pos_consensus");
                writeLatencyJson(latency, proposeLatency, "propose");
                writeLatencyJson(latency, writeLatency, "write");
                writeLatencyJson(latency, acceptLatency, "accept");

                ObjectNode batchRequests = m.putObject("batch_requests");
                batchRequests.put("avg_size", batchSize.getAverage(false));
                batchRequests.put("dp", (long) batchSize.getDP(false));

                ByteCountMessageSerializer byteCountSerializer =
                        MessageSerializerFactory.getByteCountSerializer();
                if (byteCountSerializer != null) {
                    ArrayNode messageSizes = m.putArray("message_sizes");
                    for (Map.Entry<Class<? extends SystemMessage>, ByteCountStats> e : byteCountSerializer
                            .getStats().entrySet()) {
                        long bytes = e.getValue().getByteCount();
                        long messages = e.getValue().getMessageCount();
                        ObjectNode n = messageSizes.addObject();
                        n.put("name", e.getKey().getSimpleName());
                        n.put("byte_count", bytes);
                        n.put("message_count", messages);
                    }
                }

                measurements.add(m);
            } else {
                System.out.println("Throughput = " + tp + " operations/sec (Maximum observed: " + maxTp
                                    + " ops/sec)");
                ...
                if (byteCountSerializer != null) {
                    for (Map.Entry<Class<? extends SystemMessage>, ByteCountStats> e : byteCountSerializer
                        ...
                        long messages = e.getValue().getMessageCount();
                        System.out.println(
                                e.getKey().getSimpleName() + " ByteCount = " + bytes + ":" + messages);
                    }
                }
            }
    ...
    private static double getAverageInUs(Storage storage) {
        return storage.getAverage(false) / 1000;
    }

    private static long getDpInUs(Storage storage) {
        return (long) storage.getDP(false) / 1000;
    }

    private static void writeLatencyJson(ObjectNode node, Storage storage, String keyword) {
        node.put(keyword, getAverageInUs(storage));
        node.put(keyword + "_dp", getDpInUs(storage));
        storage.reset();
    }
}

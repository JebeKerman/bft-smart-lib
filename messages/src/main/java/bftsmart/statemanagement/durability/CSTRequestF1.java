...
/**
 * This class is used to define the roles in the Collaborative State Transfer protocol. The
 * recovering replica uses this class to define which replicas should send the checkpoint, log of
 * operations lower and higher portions
 *
...
public class CSTRequestF1 extends CSTRequest {
    ...
    /**
     * Define and set the attributes of this CST Request according to the algorithm described in the
     * durability state transfer paper. In summary, the algorithm defines that, in a situation with
     * three seeders, the seeder in the middle (the one that took the checkpoint just after the
     * oldest) must send the checkpoint. The oldest must send the log of the period between the
     * middle checkpoint until the checkpoint of the newest replica. The newest replica must send
     * the log from the begging to the CID requested.
     */
    ...
    /**
     * Iterates over the others array to find the oldest replica to generate the checkpoint. In the
     * case where the oldest replica to generate the checkpoint is me, and therefore, it is not in
     * the others array, it is returned the replica with id immediate before me
     *
     * @param others the other replicas in BFT-SMaRt
     * @param cid the CID that I am requesting
     * @param globalCheckpointPeriod the global checkpoint period in which all replicas should
    ...
    private int getOldest(int[] others, int cid, int globalCheckpointPeriod, int me) {
        ...
        for (int i = 0; i < others.length; i++) {
            if (others[i] == oldestCkpReplica) return i;
        }
        return -1;
    }
